<x-app-layout>
    <div class="flex flex-col  w-full p-[32px] dark:text-gray-100">
        ...
        <x-form-modal name="edit-supplier" title="Edit Supplier" :action="route('suppliers.update', $supplier)" method="PATCH" submit-label="Save"
            ...
            <form method="POST" action="{{ route('suppliers.import', $supplier) }}" enctype="multipart/form-data"
                ...
                <div class="px-[24px] py-[20px] space-y-[16px]">
                    ...
                    <label for="import-file"
                        ...
                        <input id="import-file" name="file" type="file" :required="!stagedToken"
                            ...
                            class="sr-only">
                        <div
                            class="w-[48px] h-[48px] mx-auto rounded-full border border-[#3F7A5C]/40 flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <select id="import-strategy" name="strategy" required
                            ...
                            <option value="skip" class="text-gray-800"
                                {{ old('strategy', 'skip') === 'skip' ? 'selected' : '' }}>Skip conflicts (Default)
                            </option>
                            <option value="overwrite" class="text-gray-800"
                                {{ old('strategy') === 'overwrite' ? 'selected' : '' }}>Overwrite existing</option>
                            <option value="duplicate" class="text-gray-800"
                                {{ old('strategy') === 'duplicate' ? 'selected' : '' }}>Duplicate conflicting layups
                            </option>
                            <option value="reject" class="text-gray-800"
                                {{ old('strategy') === 'reject' ? 'selected' : '' }}>Reject entire import on conflict
                            </option>
                        </select>
                    ...
                    {{-- Dry run --}}
                    <label
                        class="flex items-center justify-between p-[12px] rounded-[8px] border border-gray-200 bg-gray-50 cursor-pointer">
                        <div class="flex items-center gap-[12px]">
                            ...
                            <div>
                                ...
                                <p class="text-[12px] text-gray-500">Simulate the import process without saving changes
                                    to the database.</p>
                            </div>
                        </div>
                            ...
                            <div class="flex items-start gap-[12px]">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-[13px] mt-[2px]">
                                        ...
                                        layer{{ count(session('import_conflicts')) === 1 ? '' : 's' }}
                                    ...
                                    <ul x-show="open" x-cloak
                                        class="mt-[8px] list-disc list-inside text-[12px] space-y-[2px]">
        ...
        <x-form-modal name="add-layup" title="Add Layup" :action="route('layups.store', $supplier)" submit-label="Create" :show="$errors->has('name') && !old('_layup_id') && !session('reopen_import')">
            <div class="text-left">
                ...
                <x-text-input id="layup-name" name="name" type="text" class="mt-1 block w-full"
                    :value="old('name')" required autofocus placeholder="Layup name" />
                <x-input-error :messages="$errors->get('name')" class="mt-2" />
            </div>
        </x-form-modal>
                    ...
                    @forelse ($layups as $layup)
                        <tr class="h-[73px] border-t border-gray-200">
                            ...
                            <td class="px-[16px] text-right">
                                ...
                                <x-form-modal name="edit-layup-{{ $layup->id }}" title="Edit Layup"
                                    :action="route('layups.update', $layup)" method="PATCH" submit-label="Save" :show="$errors->any() && (int) old('_layup_id') === $layup->id">
                                    <input type="hidden" name="_layup_id" value="{{ $layup->id }}">
                                    <div class="text-left">
                                        <x-input-label for="layup-name-{{ $layup->id }}" value="Name" />
                                        <x-text-input id="layup-name-{{ $layup->id }}" name="name"
                                            type="text" class="mt-1 block w-full" :value="old('name', $layup->name)" required />
                                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                                    </div>
                                </x-form-modal>

                                <x-modal name="delete-layup-{{ $layup->id }}" :show="false" focusable
                                    maxWidth="md">
                        ...
                        <td colspan="8" class="px-[16px] py-[12px]">
                            <div class="flex items-center justify-between">
                                <p class="text-[14px] text-gray-600">
                                    Showing <span
                                        class="font-semibold text-gray-900">{{ $layups->firstItem() ?? 0 }}</span>
